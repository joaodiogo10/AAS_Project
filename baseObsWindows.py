...
def main():
    ...
    fname=''.join(os.path.basename(file_input).split('.')[:-1])
    if method>1:
        dirname=os.path.join(os.path.dirname(__file__),"windows",fname+"_obs_s{}_m{}".format(slidingValue, method))
    else:
        dirname=os.path.join(os.path.dirname(__file__),"windows",fname+"_obs_m{}".format(method))
    os.mkdir(dirname)
    ...
    if method==1:
        ...
        obsData=seqObsWindow(np_data,lengthObsWindow[0],basename)
        print(obsData)
    elif method==2:
        print("\n\n### SLIDING Observation Windows with Length {} and Sliding {} ###".format(lengthObsWindow[0],slidingValue))
        obsData=slidingObsWindow(np_data,lengthObsWindow[0],slidingValue,basename)
        print(obsData)
    elif method==3:
        print("\n\n### SLIDING Observation Windows with Lengths {} and Sliding {} ###".format(lengthObsWindow,slidingValue))    
        obsData=slidingMultObsWindow(np_data,lengthObsWindow,slidingValue,basename)
        print(obsData)
